...
Definition per_bot : relation domain_ne := fun m n => (forall s, exists L L', {{ Rne m in s ↘ L }} /\ {{ Rne n in s ↘ L' }} /\ {{ ⊢ne L ≈ L' }}).
#[global]
Arguments per_bot /.
#[export]
...
Definition per_top : relation domain_nf := fun m n => (forall s, exists L L', {{ Rnf m in s ↘ L }} /\ {{ Rnf n in s ↘ L' }} /\ {{ ⊢nf L ≈ L' }}).
#[global]
Arguments per_top /.
#[export]
...
Definition per_top_typ : relation domain := fun a b => (forall s, exists C C', {{ Rtyp a in s ↘ C }} /\ {{ Rtyp b in s ↘ C' }} /\ {{ ⊢nf C ≈ C' }}).
#[global]
